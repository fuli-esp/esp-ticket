...
define([
  ...
  helpers.UI.toggleSidebar = function () {
    ...
    if ($sidebar.hasClass('expand')) {
      ...
      setTimeout(function () {
        Tether.position();
        $('.sidebar').find('.tether-element.tether-enabled').show();
      }, 250);
  ...
  helpers.UI.cardOverlay = function () {
    ...
    $truCard.on('click', '.tru-card-overlay-toggler', function (e) {
      ...
      if (!$(this).closest('.tru-card').hasClass('tru-card-overlay-active')) {
        $(this).html('close').closest('.tru-card').addClass('tru-card-overlay-active');
      } else {
        $(this).html('more_vert').closest('.tru-card').removeClass('tru-card-overlay-active');
      }
    });
  };
  ...
  helpers.setupBarChart = function () {
    $(document).ready(function () {
      $('.bar-chart > .bar').each(function () {
        ...
        var i = 170 * (0.01 * val);
        $this.find('span.bar-track').height(0).animate(
          {
            height: i,
          },
          1000
  ...
  helpers.formatDate = function (date, format, isUTC) {
    ...
    if (isUTC) return moment(date).utc(true).tz(timezone).format(format);
  ...
  helpers.setupTruTabs = function (tabs) {
    var toggleTab = function (element) {
      ...
      if ($(element).hasClass('active')) {
        const $highlighter = $(element).parent().find('.tru-tab-highlighter');
        ...
        setTimeout(function () {
          $this.waypoint({
            handler: function () {
              ...
              setTimeout(function () {
                $this.removeClass('hierarchical_show hierarchical_show_inView fast_animation').children().css({
